...
export async function _wms_android_Информация_о_задании(taskId: number): Promise<IResult_wms_android_Информация_о_задании> {
    ...
    for (let row of lastRecordset) {
        if (!row.error) {
            ...
            if (!(row.ВремяНачалаПлан instanceof moment)) throw new Error("результат выполнения '_wms_android_Информация_о_задании': значение в колонке 'ВремяНачалаПлан' должно быть датой (Moment)");
            ...
            if (!(row.ВремяОкончанияПлан instanceof moment)) throw new Error("результат выполнения '_wms_android_Информация_о_задании': значение в колонке 'ВремяОкончанияПлан' должно быть датой (Moment)");
            ...
            if (!(row.ВремяНачалаФакт instanceof moment)) throw new Error("результат выполнения '_wms_android_Информация_о_задании': значение в колонке 'ВремяНачалаФакт' должно быть датой (Moment)");
            ...
            if (!(row.ВремяОкончанияФакт instanceof moment)) throw new Error("результат выполнения '_wms_android_Информация_о_задании': значение в колонке 'ВремяОкончанияФакт' должно быть датой (Moment)");
...
export interface IResult_wms_android_ПИК_Взять_задание_в_работу {
    error:string;
    Ok: string
}

export async function _wms_android_ПИК_Взять_задание_в_работу(TaskId: number, KadrId: number): Promise<IResult_wms_android_ПИК_Взять_задание_в_работу> {
    if (typeof TaskId != "number") throw new Error("вызов '_wms_android_ПИК_Взять_задание_в_работу': параметр 'TaskId' должен быть числом");
    if (typeof KadrId != "number") throw new Error("вызов '_wms_android_ПИК_Взять_задание_в_работу': параметр 'KadrId' должен быть числом");
    let recordsets = await executeSql("_wms_android_ПИК_Взять_задание_в_работу " + TaskId.toString() + "," + KadrId.toString());
    let lastRecordset = recordsets[recordsets.length - 1];
    if (!lastRecordset) return { error: "_wms_android_ПИК_Взять_задание_в_работу: не вернула результатов" } as any;
    if (lastRecordset.length > 1) return { error: "_wms_android_ПИК_Взять_задание_в_работу: вернула " + lastRecordset.length + " записей вместо 1-ой" } as any;
    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ok) == "undefined") throw new Error("результат выполнения '_wms_android_ПИК_Взять_задание_в_работу': не заполнена колонка 'Ok'");
            if (typeof row.Ok != "string") throw new Error("результат выполнения '_wms_android_ПИК_Взять_задание_в_работу': значение в колонке 'Ok' должно быть строкой");            
        }
    }
...
export async function _wms_android_Логин_инфо(): Promise<IResult_wms_android_Логин_инфо> {
    ...
    for (let row of lastRecordset) {
        if (!row.error) {
            ...
            if (!(row.ВремяСервера instanceof moment)) throw new Error("результат выполнения '_wms_android_Логин_инфо': значение в колонке 'ВремяСервера' должно быть датой (Moment)");
...
export interface IResult_wms_android_РАЗГР_Взять_задание_в_работу {
    error:string;
    Ok: string
}

export async function _wms_android_РАЗГР_Взять_задание_в_работу(TaskId: number, KadrId: number): Promise<IResult_wms_android_РАЗГР_Взять_задание_в_работу> {
    if (typeof TaskId != "number") throw new Error("вызов '_wms_android_РАЗГР_Взять_задание_в_работу': параметр 'TaskId' должен быть числом");
    if (typeof KadrId != "number") throw new Error("вызов '_wms_android_РАЗГР_Взять_задание_в_работу': параметр 'KadrId' должен быть числом");
    let recordsets = await executeSql("_wms_android_РАЗГР_Взять_задание_в_работу " + TaskId.toString() + "," + KadrId.toString());
    let lastRecordset = recordsets[recordsets.length - 1];
    if (!lastRecordset) return { error: "_wms_android_РАЗГР_Взять_задание_в_работу: не вернула результатов" } as any;
    if (lastRecordset.length > 1) return { error: "_wms_android_РАЗГР_Взять_задание_в_работу: вернула " + lastRecordset.length + " записей вместо 1-ой" } as any;
    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ok) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_Взять_задание_в_работу': не заполнена колонка 'Ok'");
            if (typeof row.Ok != "string") throw new Error("результат выполнения '_wms_android_РАЗГР_Взять_задание_в_работу': значение в колонке 'Ok' должно быть строкой");            
        }
    }
...
export async function _wms_android_РАЗГР_Создать_партию_из_штрих_кода(BarCode: string, DogID: number, ClientID: number, TMC: number, ReleaseDate: Moment, ExpiredDate: Moment, PartNum: string): Promise<IResult_wms_android_РАЗГР_Создать_партию_из_штрих_кода> {
    ...
    if (!(ReleaseDate instanceof moment)) throw new Error("параметр 'ReleaseDate' должен быть датой (Moment)");
    if (!(ExpiredDate instanceof moment)) throw new Error("параметр 'ExpiredDate' должен быть датой (Moment)");
...
export async function _wms_android_РАЗГР_создать_партию(ДоговорПрихода: number, ТМЦ: number, ДатаВыпуска: Moment, СрокРеализации: Moment, ВесУпаковки: number): Promise<IResult_wms_android_РАЗГР_создать_партию> {
    ...
    if (!(ДатаВыпуска instanceof moment)) throw new Error("параметр 'ДатаВыпуска' должен быть датой (Moment)");
    if (!(СрокРеализации instanceof moment)) throw new Error("параметр 'СрокРеализации' должен быть датой (Moment)");
...
export interface IResult_wms_android_РАЗГР_Проверить_паллету {
    error:string;
    Ok: string
}

export async function _wms_android_РАЗГР_Проверить_паллету(palleteId: number, dogId: number, taskId: number, режимВводаОстаков: boolean, режимВозврата: boolean): Promise<IResult_wms_android_РАЗГР_Проверить_паллету> {
    if (typeof palleteId != "number") throw new Error("вызов '_wms_android_РАЗГР_Проверить_паллету': параметр 'palleteId' должен быть числом");
    if (typeof dogId != "number") throw new Error("вызов '_wms_android_РАЗГР_Проверить_паллету': параметр 'dogId' должен быть числом");
    if (typeof taskId != "number") throw new Error("вызов '_wms_android_РАЗГР_Проверить_паллету': параметр 'taskId' должен быть числом");
    if (typeof режимВводаОстаков != "boolean") throw new Error("вызов '_wms_android_РАЗГР_Проверить_паллету': параметр 'режимВводаОстаков' должен быть boolean");
    if (typeof режимВозврата != "boolean") throw new Error("вызов '_wms_android_РАЗГР_Проверить_паллету': параметр 'режимВозврата' должен быть boolean");
    let recordsets = await executeSql("_wms_android_РАЗГР_Проверить_паллету " + palleteId.toString() + "," + dogId.toString() + "," + taskId.toString() + "," + (режимВводаОстаков?1:0) + "," + (режимВозврата?1:0));
    let lastRecordset = recordsets[recordsets.length - 1];
    if (!lastRecordset) return { error: "_wms_android_РАЗГР_Проверить_паллету: не вернула результатов" } as any;
    if (lastRecordset.length > 1) return { error: "_wms_android_РАЗГР_Проверить_паллету: вернула " + lastRecordset.length + " записей вместо 1-ой" } as any;
    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ok) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_Проверить_паллету': не заполнена колонка 'Ok'");
            if (typeof row.Ok != "string") throw new Error("результат выполнения '_wms_android_РАЗГР_Проверить_паллету': значение в колонке 'Ok' должно быть строкой");            
        }
    }
...
export interface IResult_wms_android_РАЗГР_Взять_паллету_в_задание {
    error:string;
    Ok: string
}

export async function _wms_android_РАЗГР_Взять_паллету_в_задание(palleteId: number, taskId: number, зонаПРР: number): Promise<IResult_wms_android_РАЗГР_Взять_паллету_в_задание> {
    if (typeof palleteId != "number") throw new Error("вызов '_wms_android_РАЗГР_Взять_паллету_в_задание': параметр 'palleteId' должен быть числом");
    if (typeof taskId != "number") throw new Error("вызов '_wms_android_РАЗГР_Взять_паллету_в_задание': параметр 'taskId' должен быть числом");
    if (typeof зонаПРР != "number") throw new Error("вызов '_wms_android_РАЗГР_Взять_паллету_в_задание': параметр 'зонаПРР' должен быть числом");
    let recordsets = await executeSql("_wms_android_РАЗГР_Взять_паллету_в_задание " + palleteId.toString() + "," + taskId.toString() + "," + зонаПРР.toString());
    let lastRecordset = recordsets[recordsets.length - 1];
    if (!lastRecordset) return { error: "_wms_android_РАЗГР_Взять_паллету_в_задание: не вернула результатов" } as any;
    if (lastRecordset.length > 1) return { error: "_wms_android_РАЗГР_Взять_паллету_в_задание: вернула " + lastRecordset.length + " записей вместо 1-ой" } as any;
    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ok) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_Взять_паллету_в_задание': не заполнена колонка 'Ok'");
            if (typeof row.Ok != "string") throw new Error("результат выполнения '_wms_android_РАЗГР_Взять_паллету_в_задание': значение в колонке 'Ok' должно быть строкой");            
        }
    }
...
export interface IResult_wms_android_Типы_паллет {
    error:string;
    Ключ: number;
    Номер: string;
    Название: string;
...
export async function _wms_android_Типы_паллет(): Promise<IResult_wms_android_Типы_паллет[]> {

    let recordsets = await executeSql("_wms_android_Типы_паллет " + "");
    let lastRecordset = recordsets[recordsets.length - 1];


    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ключ) == "undefined") throw new Error("результат выполнения '_wms_android_Типы_паллет': не заполнена колонка 'Ключ'");
            if (typeof row.Ключ != "number") throw new Error("результат выполнения '_wms_android_Типы_паллет': значение в колонке 'Ключ' должно быть числом");
            if (typeof(row.Номер) == "undefined") throw new Error("результат выполнения '_wms_android_Типы_паллет': не заполнена колонка 'Номер'");
            if (typeof row.Номер != "string") throw new Error("результат выполнения '_wms_android_Типы_паллет': значение в колонке 'Номер' должно быть строкой");
            if (typeof(row.Название) == "undefined") throw new Error("результат выполнения '_wms_android_Типы_паллет': не заполнена колонка 'Название'");
            if (typeof row.Название != "string") throw new Error("результат выполнения '_wms_android_Типы_паллет': значение в колонке 'Название' должно быть строкой");
            if (typeof(row.Глубина) == "undefined") throw new Error("результат выполнения '_wms_android_Типы_паллет': не заполнена колонка 'Глубина'");
            if (typeof row.Глубина != "number") throw new Error("результат выполнения '_wms_android_Типы_паллет': значение в колонке 'Глубина' должно быть числом");
            if (typeof(row.Ширина) == "undefined") throw new Error("результат выполнения '_wms_android_Типы_паллет': не заполнена колонка 'Ширина'");
            if (typeof row.Ширина != "number") throw new Error("результат выполнения '_wms_android_Типы_паллет': значение в колонке 'Ширина' должно быть числом");            
        }
    }
...
export async function _wms_android_РАЗГР_завершить_паллету(taskId: number, palleteId: number, palleteType: number, ширина: number, глубина: number, высота: number, неперемещаемая: boolean, режимВводаОстатков: boolean): Promise<IResult_wms_android_РАЗГР_завершить_паллету> {
    ...
    let recordsets = await executeSql("_wms_android_РАЗГР_завершить_паллету " + taskId.toString() + "," + palleteId.toString() + "," + palleteType.toString() + "," + ширина.toString() + "," + глубина.toString() + "," + высота.toString() + "," + (неперемещаемая?1:0) + "," + (режимВводаОстатков?1:0));
    let lastRecordset = recordsets[recordsets.length - 1];
    ...
    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ok) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_завершить_паллету': не заполнена колонка 'Ok'");
            if (typeof row.Ok != "string") throw new Error("результат выполнения '_wms_android_РАЗГР_завершить_паллету': значение в колонке 'Ok' должно быть строкой");            
        }
    }
...
export interface IResult_wms_android_РАЗГР_инфо_для_отката {
    error:string;
    Ключ: number;
    ТМЦ: number;
    Партия: number;
...
export async function _wms_android_РАЗГР_инфо_для_отката(Задание: number, Паллета: number): Promise<IResult_wms_android_РАЗГР_инфо_для_отката[]> {
    if (typeof Задание != "number") throw new Error("вызов '_wms_android_РАЗГР_инфо_для_отката': параметр 'Задание' должен быть числом");
    if (typeof Паллета != "number") throw new Error("вызов '_wms_android_РАЗГР_инфо_для_отката': параметр 'Паллета' должен быть числом");
    let recordsets = await executeSql("_wms_android_РАЗГР_инфо_для_отката " + Задание.toString() + "," + Паллета.toString());
    let lastRecordset = recordsets[recordsets.length - 1];


    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ключ) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': не заполнена колонка 'Ключ'");
            if (typeof row.Ключ != "number") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': значение в колонке 'Ключ' должно быть числом");
            if (typeof(row.ТМЦ) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': не заполнена колонка 'ТМЦ'");
            if (typeof row.ТМЦ != "number") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': значение в колонке 'ТМЦ' должно быть числом");
            if (typeof(row.Партия) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': не заполнена колонка 'Партия'");
            if (typeof row.Партия != "number") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': значение в колонке 'Партия' должно быть числом");
            if (typeof(row.Количество) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': не заполнена колонка 'Количество'");
            if (typeof row.Количество != "number") throw new Error("результат выполнения '_wms_android_РАЗГР_инфо_для_отката': значение в колонке 'Количество' должно быть числом");            
        }
    }
...
export interface IResult_wms_android_РАЗГР_откат {
    error:string;
    Ok: string
}

export async function _wms_android_РАЗГР_откат(скл_Комплектация_Ключ: number): Promise<IResult_wms_android_РАЗГР_откат> {
    if (typeof скл_Комплектация_Ключ != "number") throw new Error("вызов '_wms_android_РАЗГР_откат': параметр 'скл_Комплектация_Ключ' должен быть числом");
    let recordsets = await executeSql("_wms_android_РАЗГР_откат " + скл_Комплектация_Ключ.toString());
    let lastRecordset = recordsets[recordsets.length - 1];
    if (!lastRecordset) return { error: "_wms_android_РАЗГР_откат: не вернула результатов" } as any;
    if (lastRecordset.length > 1) return { error: "_wms_android_РАЗГР_откат: вернула " + lastRecordset.length + " записей вместо 1-ой" } as any;
    for (let row of lastRecordset) {
        if (!row.error) {
            if (typeof(row.Ok) == "undefined") throw new Error("результат выполнения '_wms_android_РАЗГР_откат': не заполнена колонка 'Ok'");
            if (typeof row.Ok != "string") throw new Error("результат выполнения '_wms_android_РАЗГР_откат': значение в колонке 'Ok' должно быть строкой");            
        }
    }
